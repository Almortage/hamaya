...
$usernn = "ALMORTAGEL_12";
$userbot = str_replace("ALMORTAGEL_12","",$usernn);
$admiin = "5089553588";
define("API_KEY","5688348631:AAEv0IrhNSNYM8fxv9CWuR9FgNu-7JC6A5U");
...
$coss = json_decode(file_get_contents("data/ALMORTAGEL.json"),1);
$malke = $coss['malk'];
if($malke == null){
$malkei = $info['id'];
}elseif($malke != null){
...
#############
$admin = "$admiin";
$Dev = array($admiin,"5089553588");
$eri = array($admiin,"5089553588");
$idbot = "5688348631";
$sudo = "$admin";
$buy = "$userbot";
$DevUser = "@$buy";
$devchink = "$namch";
$userbot = "Almortagel20_bot";
$chsource = "$devchink";
...
[['text' => "✧ - قناة تحديثات البوت", 'url' =>"https://t.me/$namch"]],
...
⋄︙ [تحديثات السورس الجديده](https://t.me/$namch)",
'parse_mode'=>"markdown",
...
$Dev = array("5089553588");
$chtesbot = file_get_contents("chteosbot.txt");
$chtbot = file_get_contents("chtbot.txt");
if ($text == "⋄ تعين قناة السورس " and in_array($from_id,$Dev)){
file_put_contents("chteosbot.txt","namch");
bot("sendMessage",[
"chat_id"=>$chat_id,
"text"=>"🌼 | حسنا عزيزي المبرمج ارسل يوزر قناتك بدون @",
'parse_mode'=>"MARKDOWN",
 'reply_to_message_id'=>$message_id
,]);}
if($text && $chtesbot =="namch" and in_array($from_id,$Dev)){
file_put_contents("chtbot.txt",$text); 
file_put_contents("chteosbot.txt","");
bot("sendmessage",[
"chat_id"=>$chat_id,
"text" => "☘️ | عزيزي المبرمج تم اضافة قناة السورس",
'parse_mode'=>"MARKDOWN",
 'reply_to_message_id'=>$message_id
,]);}
if( $text=="⋄ قسم تواصل"){
...
$nazarr = explode ("\n",$nazarr_infos);
/*===== dev ~ @Almortagel20_bot =====*/
$ALMORTAGELs_info = file_get_contents("data/ALMORTAGEL/$chat_id.txt");
$ALMORTAGEL  = explode("\n",$ALMORTAGELs_info);
$ALMORTAGELs_infos = file_get_contents("data/ALMORTAGEL/$chat_id/crlo.txt");
$ALMORTAGELs = explode ("\n",$ALMORTAGELs_infos);
/*===== dev ~ @Almortagel20_bot =====*/
...
mkdir("data/ALMORTAGEL");
mkdir("data/ALMORTAGEL/$chat_id");
mkdir("data/manger");
...
elseif(in_array($from_id,$ALMORTAGEL) ){
$info = "منشئ";
}
...
if($status == 'creator' or in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$ALMORTAGEL) || in_array($from_id,$nazar) || in_array($from_id,$eri)){
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"*⋄︙ارسل الامر الجديد الان .*",'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message_id,
...
$REMAS= str_replace('رفع مطور ثانوي ','',$text);
$ALMORTAGELid = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$REMAS"))->result->username;
$ALMORTAGELname = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$REMAS"))->result->first_name;
file_put_contents("data/developers/developer.txt",$REMAS ."\n " , FILE_APPEND);
file_put_contents("data/developers/developers.txt",'[@'.$ALMORTAGELid ."]". "\n " , FILE_APPEND);
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"⋄︙العضو ⋙「 [$ALMORTAGELname](tg://user?id=$REMAS) 」 
⋄︙تم ترقية ليصبح مطور ثانوي
",'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true,
]);
}
...
$REMASID= str_replace('تنزيل مطور ثانوي ','',$text);
$ALMORTAGELid = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$REMASID"))->result->username;
$ALMORTAGELname = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$REMASID"))->result->first_name;
...
file_put_contents("data/developers/developers.txt",$str);
bot('SendMessage',['chat_id'=>$chat_id, 'text'=>"
⋄︙ العضو ⋙「 [$ALMORTAGELname](tg://user?id=$REMASID) 」 
⋄︙تم تنزيله من قائمة المطورين الثانوي
", 'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true, ]); }}
...
if($rt && $text == "رفع منشئ" or $rt && $text == "من" and $text==$settings["information"]["mans"] and !in_array($re_id,$manger)){
			file_put_contents("data/ALMORTAGEL/$chat_id.txt",$re_id . "\n" , FILE_APPEND);
			file_put_contents("data/ALMORTAGEL/$chat_id/crlo.txt" , " *𓆩* [" . "@". $re_user ."] *𓆪* " . "»" . " *𓆩* `". $re_id ."` *𓆪* ". "\n" , FILE_APPEND);
bot('SendMessage',[
'chat_id'=>$chat_id,
'text'=>"⋄︙العضو ⋙「 [$re_name](tg://user?id=$re_id) 」 
⋄︙تم ترقية ليصبح منشئ
...
elseif($rt && $text == "رفع منشئ" or $rt && $text == "من" and $text == "رفع منشئ" and $text == "من" and $text==$settings["information"]["mans"] and in_array($re_id,$ALMORTAGEL)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"⋄︙العضو ⋙「 [$re_name](tg://user?id=$re_id) 」 
⋄︙ هوه بالفعل منشئ 
",
...
$IDALMORTAGEL = str_replace('رفع منشئ ','',$text);
if ($text =="رفع منشئ $IDALMORTAGEL" and preg_match('/([0-9])/i',$IDALMORTAGEL) and !in_array($IDALMORTAGEL,$ALMORTAGEL)){
$IDALMORTAGEL= str_replace('رفع منشئ ','',$text);
$ALMORTAGELID = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDALMORTAGEL"))->result->username;
$ALMORTAGELNAME = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDALMORTAGEL"))->result->first_name;
			file_put_contents("data/ALMORTAGEL/$chat_id.txt",$IDALMORTAGEL . "\n" , FILE_APPEND);
			file_put_contents("data/ALMORTAGEL/$chat_id/crlo.txt" , " *𓆩* [" . "@". $ALMORTAGELID ."] *𓆪* " . "»" . " *𓆩* `". $IDALMORTAGEL ."` *𓆪* ". "\n" , FILE_APPEND);
bot('SendMessage',[
'chat_id'=>$chat_id,
'text'=>"⋄︙لعضو ⋙「 [$ALMORTAGELNAME](tg://user?id=$IDALMORTAGEL) 」 
⋄︙ تم ترقية ليصبح منشئ
"
,'parse_mode'=>'markdown',
'reply_to_message_id'=>$message->message_id,
...
$ALMORTAGELNAME = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDALMORTAGEL"))->result->first_name;
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"⋄︙العضو ⋙「 [$ALMORTAGELNAME](tg://user?id=$IDALMORTAGEL) 」 
⋄︙هوه بالفعل منشئ 
",
'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true,
]);
...
$IDALMORTAGEL = str_replace('تنزيل منشئ ','',$text);
if ($text =="تنزيل منشئ $IDALMORTAGEL" and preg_match('/([0-9])/i',$IDALMORTAGEL) and in_array($IDALMORTAGEL,$ALMORTAGEL)){
$IDALMORTAGEL= str_replace('تنزيل منشئ ','',$text);
$ALMORTAGELID = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDALMORTAGEL"))->result->username;
$ALMORTAGELNAME = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDALMORTAGEL"))->result->first_name;
	$IDALMORTAGEL_info = file_get_contents("data/ALMORTAGEL/$chat_id.txt");
	$mdrs = file_get_contents("data/ALMORTAGEL/$chat_id/crlo.txt");
	$mdrs1 = explode("             \n",$mdrs);
	$str = str_replace($IDALMORTAGEL,"",$IDALMORTAGEL_info);
	$str2 = str_replace(" *𓆩* [" . "@". $ALMORTAGELID ."] *𓆪* " . "»" . " *𓆩* `". $IDALMORTAGEL ."` *𓆪* ","",$mdrs1);
	file_put_contents("data/ALMORTAGEL/$chat_id.txt",$str);
	file_put_contents("data/ALMORTAGEL/$chat_id/crlo.txt",$str2);
	bot('SendMessage',['chat_id'=>$chat_id,
    'text'=>"⋄︙العضو ⋙「 [$ALMORTAGELNAME](tg://user?id=$IDALMORTAGEL) 」 
...
file_put_contents("data/ALMORTAGEL/$chat_id.txt","");
file_put_contents("data/ALMORTAGEL/$chat_id.txt","");
file_put_contents("data/ALMORTAGEL/$chat_id/crlo.txt" ,"");
bot('SendMessage',['chat_id'=>$chat_id,
...
if($re and $text == "تنزيل المنشئ" || $text == "تنزيل منشئ"  and in_array($re_id,$ALMORTAGEL)){
	$re_id_info = file_get_contents("data/ALMORTAGEL/$chat_id.txt");
	$mdrs = file_get_contents("data/ALMORTAGEL/$chat_id/crlo.txt");
	$mdrs1 = explode("             \n",$mdrs);
	$str = str_replace($re_id,"",$re_id_info);
	$str2 = str_replace(" *𓆩* [" . "@". $re_user ."] *𓆪* " . "»" . " *𓆩* `". $re_id ."` *𓆪* ","",$mdrs1);
...
if($re and $text == "تنزيل المنشئ" || $text == "تنزيل منشئ" || $text == "ت م" || $text == "تنم" and !in_array($re_id,$ALMORTAGEL)){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
⋄︙ العضو ⋙「 [$re_name](tg://user?id=$re_id) 」 
⋄︙ ليس منشئ ليتم تنزيله 
...
if($text == "المنشئين" || $text == "قائمه المنشئين" and $ALMORTAGELs_info != NULL and $ALMORTAGELs_info != " "){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"⋄︙اليك قائمة ⋙ المشئينن
$ALMORTAGELs_infos\n",'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true,
]);
}
if($text == "المنشيئن" ||  $text == "قائمه المنشيئن" and $ALMORTAGELs_info == NULL || $ALMORTAGELs_info == " " || $ALMORTAGELs_info == ""){
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"⋄︙عزيزي ⋙「 [$first_name](tg://user?id=$from_id) 」 
⋄︙لايوجد منشئ حاليأ
",'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true,
...
if($status == "creator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$ALMORTAGEL) || in_array($from_id,$eri)  || in_array($from_id,$nazar)) {
...
if($status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$AUBEHAB) || in_array($from_id,$ALMORTAGEL) || in_array($from_id,$eri) || in_array($from_id,$nazar)) {
...
$IDALMORTAGEL = str_replace('رفع ادمن ','',$text);
if ($text =="رفع ادمن $IDALMORTAGEL" and preg_match('/([0-9])/i',$IDALMORTAGEL) and !in_array($IDALMORTAGEL,$admin_user)){
$IDALMORTAGEL= str_replace('رفع ادمن ','',$text);
$ALMORTAGELID = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDALMORTAGEL"))->result->username;
$ALMORTAGELNAME = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDALMORTAGEL"))->result->first_name;
	if($settings["lock"]["rfaabot"] == "مقفول"){
			file_put_contents("data/admin_user/$chat_id.txt",$IDALMORTAGEL . "\n" , FILE_APPEND);
			file_put_contents("data/admin_user/$chat_id/mange.txt" , " *𓆩* [" . "@". $ALMORTAGELID ."] *𓆪* " . "»" . " *𓆩* `". $IDALMORTAGEL ."` *𓆪* ". "\n" , FILE_APPEND);
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
⋄︙ العضو ⋙「 [$ALMORTAGELNAME](tg://user?id=$IDALMORTAGEL) 」 
⋄︙ تم ترقية ليصبح ادمن
",'parse_mode'=>'markdown','reply_to_message_id'=>$message->message_id,'disable_web_page_preview'=>true,
]);
}
...
if ($text =="رفع ادمن $IDALMORTAGEL" and preg_match('/([0-9])/i',$IDALMORTAGEL) and in_array($IDALMORTAGEL,$admin_user)){
$IDALMORTAGEL= str_replace('رفع ادمن ','',$text);
$ALMORTAGELID = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDKLOS"))->result->username;
$ALMORTAGELNAME = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDKLOS"))->result->first_name;
bot('SendMessage',['chat_id'=>$chat_id,
'text'=>"
⋄︙ العضو ⋙「 [$ALMORTAGELNAME](tg://user?id=$IDALMORTAGEL) 」 
⋄︙ هوه بالفعل ادمن
...
$IDALMORTAGEL = str_replace('تنزيل ادمن ','',$text);
if ($text =="تنزيل ادمن $IDALMORTAGEL" and preg_match('/([0-9])/i',$IDALMORTAGEL) and in_array($IDALMORTAGEL,$admin_user)){
$IDALMORTAGEL= str_replace('تنزيل ادمن ','',$text);
$ALMORTAGELID = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDALMORTAGEL"))->result->username;
$ALMORTAGELNAME = json_decode(file_get_contents("http://api.telegram.org/bot$tk/getChat?chat_id=$IDALMORTAGEL"))->result->first_name;
	$IDALMORTAGEL_info = file_get_contents("data/admin_user/$chat_id.txt");
	$admn = file_get_contents("data/admin_user/$chat_id/mange.txt");
	$admn1 = explode("             \n",$admn);
	$str = str_replace($IDALMORTAGEL,"",$IDALMORTAGEL_info);
	$str2 = str_replace(" *𓆩* [" . "@". $ALMORTAGELID ."] *𓆪* " . "»" . " *𓆩* `". $IDALMORTAGEL ."` *𓆪* ","",$admn1);
...
if($status == "creator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$AUBEHAB) || in_array($from_id,$admin_user) || in_array($from_id,$manger) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri) || in_array($from_id,$nazar)) {
...
if($rt && $text == "تنزيل الكل" or $rt && $text == "تنزيل من كل الرتب" and $text==$settings["information"]["tkal"]){
if(in_array($from_id,$ALMORTAGEL)) {
	$re_id_info = file_get_contents("data/manger/$chat_id.txt");
	$mdrs = file_get_contents("data/manger/$chat_id/mange.txt");
	$mdrs1 = explode("             \n",$mdrs);
	$str = str_replace($re_id,"",$re_id_info);
...
if($status == "creator" and in_array($from_id,$Dev) and in_array($from_id,$eri) and in_array($from_id,$ALMORTAGEL) and in_array($from_id,$nazar)){
...
if ($tc == 'group' | $tc == 'supergroup'){
if ($status !=  creator  && $status !=  administrator  && !in_array($from_id,$Dev) && !in_array($from_id,$manger) && !in_array($from_id,$admin_user) && !in_array($from_id,$developer) && !in_array($from_id,$ALMORTAGEL) && !in_array($from_id,$eri) && !in_array($from_id,$nazar)){
...
if ($status !=  creator  && $status !=  administrator  && !in_array($from_id,$Dev)  && !in_array($from_id,$mmyaz) && !in_array($from_id,$manger) && !in_array($from_id,$admin_user) && !in_array($from_id,$developer) && !in_array($from_id,$ALMORTAGEL) && !in_array($from_id,$eri) && !in_array($from_id,$nazar)){
...
if ($status !=  creator  && !in_array($from_id,$Dev)  && !in_array($from_id,$mmyaz) && !in_array($from_id,$manger) && !in_array($from_id,$admin_user) && !in_array($from_id,$developer) && !in_array($from_id,$ALMORTAGEL) && !in_array($from_id,$eri) && !in_array($from_id,$nazar)){
bot('deletemessage',[
'chat_id'=>$chat_id,
'message_id'=>$message_id
]);
...
if ( $you != 'creator' && $you != 'administrator' && $edit_for_id != $Dev && $edit_for_id != $manger && $edit_for_id != $admin_user && $edit_for_id != $developer && $edit_for_id != $ALMORTAGEL && $edit_for_id != $eri && $edit_for_id != $nazar){
...
if($text =="قفل الروابط" ){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri) or in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$AUBEHAB) or in_array($from_id,$eri) or in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
if ( $status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$AUBEHAB) or in_array($from_id,$eri) or in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri) or in_array($from_id,$AUBEHAB) or in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$AUBEHAB) or in_array($from_id,$eri)) {$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"*
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$AUBEHAB) or in_array($from_id,$eri)) {$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
elseif($rt && $text== "حذف"){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$AUBEHAB) or in_array($from_id,$eri) or in_array($from_id,$nazar)) { bot('deletemessage',[
 'chat_id'=>$chat_id,
 'message_id'=>$re_msgid
 ]);
 bot('deletemessage',[
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$AUBEHAB) or in_array($from_id,$eri) or in_array($from_id,$nazar)) {$num = str_replace(['/rmsg ','تنظيف'],'',$text );
if ($num <= 300 && $num >= 1){
$add = $settings["information"]["added"];
if ($add == true) {
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$AUBEHAB) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {$newname= str_replace(['ضع اسم '],'',$text );
 bot('setChatTitle',[
 'chat_id'=>$chat_id,
 'title'=>$newname
]);
...
if($status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$AUBEHAB) || in_array($from_id,$developer) || in_array($from_id,$ALMORTAGEL) || in_array($from_id,$eri) || in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {	
	bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$eri) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar)){
...
if($status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$eri) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar)) {if ($tc == 'group' | $tc == 'supergroup'){
$add = $settings["information"]["added"];
if ($add == true) {
bot('sendMessage',[
'chat_id'=>$chat_id,
...
$HAMD = file_get_contents("data/ALMORTAGEL.json");
...
if ($status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
⋄︙اهلا بك عزيزي
...
file_put_contents("data/ALMORTAGEL.json","ALMORTAGEL");
}}
if($text and $HAMD =="ALMORTAGEL" and $HHAMDD == $from_id){
if ($status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
if ( $status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {$add = $settings["information"]["added"];
if ($add == true) {
...
if ( $status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
if ($text =="تفعيل الطرد" ){
if($status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer)|| in_array($from_id,$ALMORTAGEL) || in_array($from_id,$AUBEHAB) || in_array($from_id,$eri)) {$add = $settings["information"]["added"];
if ($add == true) {	
	bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"
...
if ( $status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {
if ( $statusrt != 'creator' && !in_array($re_id,$Dev) && !in_array($re_id,$manger) && !in_array($re_id,$mmyaz) && !in_array($re_id,$admin_user) && !in_array($re_id,$mmyaz) && !in_array($re_id,$developer) && !in_array($re_id,$ALMORTAGEL) && !in_array($re_id,$nazar) && !in_array($re_id,$eri)) {
bot('kickChatMember',[
'chat_id'=>$chat_id,
'user_id'=>$re_id
]);
...
if ( $status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {
if ( $statusrt != 'creator' && !in_array($kickuser,$Dev) && !in_array($kickuser,$manger) && !in_array($kickuser,$mmyaz) && !in_array($kickuser,$admin_user) && !in_array($kickuser,$mmyaz) && !in_array($kickuser,$developer) && !in_array($kickuser,$ALMORTAGEL) && !in_array($kickuser,$nazar) && !in_array($kickuser,$eri)) {
bot('kickChatMember',[
'chat_id'=>$chat_id,
'user_id'=>$re_id
]);
...
elseif($status == "creator" || $status == "administrator" or in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$admin_user) || in_array($from_id,$manger) || in_array($from_id,$nazar) || in_array($from_id,$ALMORTAGEL) || in_array($from_id,$eri)) {
...
elseif(in_array($re_id,$ALMORTAGEL) ){
$infor = "المنشئ";
}
...
file_put_contents("data/namebot.txt","المرتجل");
...
$botproxre = array(
"لك نته لبوت",
"ياخي ورب اسمي المرتجل",
"حبي فدوه لصيح بوت اسمي المرتجل",
"اسمي المرتجل حياتي"
);
...
$abuehab = array("هلا قلبي معاك المرتجل","تفضل حبيبي انا المرتجل🌝💞","يمك حياتي امرني 🥺💞","شتريد كل شويه المرتجل","يابه نجب كل شويه المرتجل خبصتنه");
$ehab = array_rand($proxre, 1);
if($text == "المرتجل" and $namebot == NULL){
if ($tc == 'group' | $tc == 'supergroup'){
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
⋄︙اهلا بك عزيزي
...
elseif(in_array($BANID,$ALMORTAGEL) ){
$inf = "المنشئ";
}
...
$BANID= str_replace('حظر ','',$text);
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$AUBEHAB) or in_array($from_id,$eri)) {
if ( $REMAS2 != 'creator' && $REMAS2 != 'administrator' && !in_array($BANID,$Dev) && !in_array($BANID,$manger) && !in_array($BANID,$admin_user) && !in_array($BANID,$mmyaz) && !in_array($BANID,$developer) && !in_array($BANID,$nazar) && !in_array($BANID,$ALMORTAGEL) && !in_array($BANID,$eri) && !in_array($BANID,$AUBEHAB)) {
	bot('KickChatMember',[
    'chat_id'=>$chat_id,
    'user_id'=>$REMAS
      ]);
...
'reply_markup'=>$inlinebutton,
 ...
 if($rt && $text == "حظر" or $rt && $text == "احظر" and $text==$settings["information"]["kik"] ){
 	if ($settings["lock"]["banuser"] =="مقفول"){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$AUBEHAB) or in_array($from_id,$eri)) {
if ( $statusrt != 'creator' && $statusrt != 'administrator' && !in_array($re_id,$Dev) && !in_array($re_id,$manger) && !in_array($re_id,$admin_user) && !in_array($re_id,$mmyaz) && !in_array($re_id,$developer) && !in_array($re_id,$nazar) && !in_array($re_id,$ALMORTAGEL) && !in_array($re_id,$eri) && !in_array($re_id,$AUBEHAB)) {
	bot('KickChatMember',[
    'chat_id'=>$chat_id,
    'user_id'=>$re_id
      ]);
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$AUBEHAB) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {
...
if ($tc == 'group' | $tc == 'supergroup'){  
if ( $status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {
$add = $settings["information"]["added"];
if ($add == true) {
...
if ($status !=  creator  && $status !=  administrator  && !in_array($from_id,$Dev) && !in_array($from_id,$manger) && !in_array($from_id,$admin_user) && !in_array($from_id,$mmyaz) && !in_array($from_id,$developer) && !in_array($from_id,$eri) && !in_array($from_id,$nazar) && !in_array($from_id,$ALMORTAGEL) ){
if ($tc == 'group' | $tc == 'supergroup'){
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$AUBEHAB) or in_array($from_id,$developer) or in_array($from_id,$nazar) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri)) {
if($text == "عدد الميديا"){
bot('sendMessage',[
'chat_id'=>$chat_id,
'text'=>"
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)) {$add = $settings["information"]["added"];
if ($add == true) {
 bot('sendmessage',[
 'chat_id'=>$chat_id,
 'text'=>"⋄︙بواسطة ⋙ [$first_name](tg://user?id=$from_id)
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)) {
if ($text == "قفل الدخول" ) {
bot('sendMessage',[
     'chat_id'=>$chat_id,
     'text'=>"⋄︙بواسطة ⋙ [$first_name](tg://user?id=$from_id)
...
if($text =="من سيربح المليون" or $data =="milon"){
...
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[['text'=>"لعبة الرياضات",'url'=>"https://t.me/gamebot?game=MathBattle"],['text'=>"لعبة XO",'url'=>"http://t.me/xoBot?play"]],
...
[['text'=>'⋄ من سيربح المليون ⋄' ,'callback_data'=>"milon"]],
[['text'=>'⋄ قريبأ ⋄' ,'callback_data'=>"OFF"],['text'=>'⋄ قريبأ ⋄' ,'callback_data'=>"OFF"]],
[['text'=>'⋄ الغاء ⋄' ,'callback_data'=>"deletmil"]],
[['text'=>"⋄ قناة السورس ⋄",'url'=>"t.me/$namch"]],
]])
]); 
}
}
...
$time = date('h:i A');
date_default_timezone_set('Asia/Baghdad');
$idALMORTAGEL = file_get_contents("data/ALMORTAGEL.json");
if($re and $text == "تنزيل" and $text == "رفع"){
file_put_contents("data/ALMORTAGEL.json","$re_id");
...
if (!in_array($idALMORTAGEL,$nazar)){
$userr = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->username;
$namee = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->first_name;
file_put_contents("data/nazar/$chat_id.txt",$idALMORTAGEL . "\n" , FILE_APPEND);
file_put_contents("data/nazar/$chat_id/nazr.txt" , " *𓆩* [" . "@". $userr ."] *𓆪* " . "»" . " *𓆩* `". $idALMORTAGEL ."` *𓆪* ". "\n" , FILE_APPEND);
bot('EditMessageText',[
'chat_id'=>$chat_id2,
'message_id'=>$message_id2,
'text'=>"⋄︙العضو ← [$namee](tg://user?id=$idALMORTAGEL)
...
$re_id_info = file_get_contents("data/ALMORTAGEL/$chat_id.txt");
	$mdrs = file_get_contents("data/ALMORTAGEL/$chat_id/crlo.txt");
	$mdrs1 = explode("             \n",$mdrs);
	$str = str_replace($idALMORTAGEL,"",$re_id_info);
	$str2 = str_replace(" *𓆩* [" . "@". $userr ."] *𓆪* " . "»" . " *𓆩* `". $idALMORTAGEL ."` *𓆪* ","",$mdrs1);
	file_put_contents("data/ALMORTAGEL/$chat_id.txt",$str);
	file_put_contents("data/ALMORTAGEL/$chat_id/crlo.txt",$str2);
bot('EditMessageText',[
'chat_id'=>$chat_id2,
...
if (!in_array($idALMORTAGEL,$ALMORTAGEL)){
$userr = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->username;
$namee = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->first_name;
file_put_contents("data/ALMORTAGEL/$chat_id.txt",$idALMORTAGEL . "\n" , FILE_APPEND);
file_put_contents("data/ALMORTAGEL/$chat_id/crlo.txt" , " *𓆩* [" . "@". $userr ."] *𓆪* " . "»" . " *𓆩* `". $idALMORTAGEL ."` *𓆪* ". "\n" , FILE_APPEND);
bot('EditMessageText',[
'chat_id'=>$chat_id2,
'message_id'=>$message_id2,
'text'=>"⋄︙العضو ← [$namee](tg://user?id=$idALMORTAGEL)
...
if($data == "md"){
if ( $status == 'creator' or in_array($from_id2,$Dev) or in_array($from_id2,$developer) or in_array($from_id2,$ALMORTAGEL) or in_array($from_id2,$eri) or in_array($from_id2,$nazar)) {
if (!in_array($idALMORTAGEL,$manger)){
$userr = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->username;
$namee = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->first_name;
file_put_contents("data/manger/$chat_id.txt",$idALMORTAGEL . "\n" , FILE_APPEND);
file_put_contents("data/manger/$chat_id/mange.txt" , " *𓆩* [" . "@". $userr ."] *𓆪* " . "»" . " *𓆩* `". $idALMORTAGEL ."` *𓆪* ". "\n" , FILE_APPEND);
bot('EditMessageText',[
'chat_id'=>$chat_id2,
'message_id'=>$message_id2,
'text'=>"⋄︙العضو ← [$namee](tg://user?id=$idALMORTAGEL)
...
if($data == "ad"){
if ( $status == 'creator' or in_array($from_id2,$Dev) or in_array($from_id2,$manger) or in_array($from_id2,$developer) or in_array($from_id2,$ALMORTAGEL) or in_array($from_id2,$eri) or in_array($from_id2,$nazar)) {
if (!in_array($idALMORTAGEL,$admin_user)){
$userr = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->username;
$namee = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->first_name;
file_put_contents("data/admin_user/$chat_id.txt",$idALMORTAGEL . "\n" , FILE_APPEND);
file_put_contents("data/admin_user/$chat_id/mange.txt" , " *𓆩* [" . "@". $userr ."] *𓆪* " . "»" . " *𓆩* ". $idALMORTAGEL ." *𓆪* ". "\n" , FILE_APPEND);
bot('EditMessageText',[
'chat_id'=>$chat_id2,
'message_id'=>$message_id2,
'text'=>"⋄︙العضو ← [$namee](tg://user?id=$idALMORTAGEL)
...
$re_id_info = file_get_contents("data/admin_user/$chat_id.txt");
 ...
 $admn1 = explode("             \n",$admn);
 $str = str_replace($idALMORTAGEL,"",$re_id_info);
 $str2 = str_replace(" *𓆩* [" . "@". $userr ."] *𓆪* " . "»" . " *𓆩* ". $idALMORTAGEL ." *𓆪* ","",$admn1);
 file_put_contents("data/admin_user/$chat_id.txt",$str);
 file_put_contents("data/admin_user/$chat_id/mange.txt",$str2);
bot('EditMessageText',[
'chat_id'=>$chat_id2,
...
if($data == "mz"){
if ( $status == 'creator' or in_array($from_id2,$Dev) or in_array($from_id2,$manger) or in_array($from_id2,$admin_user) or in_array($from_id2,$developer) or in_array($from_id2,$ALMORTAGEL) or in_array($from_id2,$eri) or in_array($from_id2,$nazar)) {
if (!in_array($idALMORTAGEL,$mmyaz)){
$userr = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->username;
$namee = json_decode(file_get_contents("http://api.telegram.org/bot$token/getChat?chat_id=$idALMORTAGEL"))->result->first_name;
file_put_contents("data/mmyaz/$chat_id.txt",$idALMORTAGEL . "\n" , FILE_APPEND);
file_put_contents("data/mmyaz/$chat_id/mange.txt" , " *𓆩* [" . "@". $userr ."] *𓆪* " . "»" . " *𓆩* `". $idALMORTAGEL ."` *𓆪* ". "\n" , FILE_APPEND);
bot('EditMessageText',[
'chat_id'=>$chat_id2,
'message_id'=>$message_id2,
'text'=>"⋄︙العضو ← [$namee](tg://user?id=$idALMORTAGEL)
...
bot('sendphoto',[
'chat_id'=>$chat_id,
'photo'=>"https://t.me/DEV_JABWA/236",
'reply_to_message_id'=>$message->message_id, 
]);
}}
if($text == "ضحك"){
...
bot('sendphoto',[
'chat_id'=>$chat_id,
'photo'=>"https://t.me/DEV_JABWA/240",
'reply_to_message_id'=>$message->message_id, 
]);
}}
if($text == "حبك"){
...
if ( $rt && $text =="تثبيت"){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)) { bot('pinChatMessage',[
    'chat_id'=>$chat_id,
    'message_id'=>$replyid
      ]);
bot('sendmessage',[
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)) { bot('unpinChatMessage',[
    'chat_id'=>$chat_id,
    'message_id'=>$replyid
      ]);
bot('sendmessage',[
...
if ($text =="تفعيل البحث" ){
if($status == "creator" ||  $status == "administrator" ||  in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$ALMORTAGEL) || in_array($from_id,$eri) || in_array($from_id,$nazar)) {$add = $settings["information"]["added"];
if ($add == true) {	
	bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)) {$add = $settings["information"]["added"];
if ($add == true) {	
	bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"
...
⋄︙[تعليمات الامر (مهم)](https://t.me/$namch)
",'parse_mode'=>"MARKDOWN",
 'reply_to_message_id'=>$message_id
,]);}
...
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"⋄︙اوامر الحمايه لسورس المرتجل
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
⋄︙قفل ، فتح ← الامر 
...
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"⋄︙اوامر الاعضاء لسورس المرتجل
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
...
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
⋄︙قناة البوت ← [$devchink]";
$m6 = "⋄︙اوامر الاعضاء لسورس المرتجل
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
⋄︙تحويل/بالرد - لتحويل صيغ 
⋄︙ملصق ← صوره
⋄︙صوره ← ملصق
...
date_default_timezone_set('Asia/Baghdad');
if($text =="الاوامر"){
if ( $status == 'creator' or in_array($from_id2,$Dev) or in_array($from_id2,$manger) or in_array($from_id2,$admin_user) or in_array($from_id2,$developer) or in_array($from_id2,$nazar) or in_array($from_id2,$ALMORTAGEL) or in_array($from_id2,$eri)) {
if ($tc == 'group' | $tc == 'supergroup'){
$add = $settings["information"]["added"];
if ($add == true) {
bot('sendmessage',[
...
$tofALMORTAGEL = array("خشوف وجهه يستاهل تفله","دي لك يخره شوف وجهك حرامت اذب تفلتي عليه😈👋🏿","دمشي لاتفل بخشمك🥱👏🏿","لك ياحيوان حترم لا اهينك اتفل بعينك😟🤘🏿","انته شايف وجهكك ب امرايه☹️🤘🏿");
$tofALMORTAGEL2 = array_rand($tofALMORTAGEL, 1);
...
bot('sendMessage',[ 'chat_id'=>$chat_id, 'text'=>"$tofALMORTAGEL[$tofALMORTAGEL2]",'parse_mode'=>"MARKDOWN", 'reply_to_message_id'=>$message->reply_to_message->message_id ]); } }
...
bot('sendmessage',['chat_id'=>$chat_id,'photo'=>"https://telegra.ph/file/14c7948ad180050fe16e4.jpg",
'caption'=>"
",'parse_mode'=>"MarkDown",'disable_web_page_preview'=>true,'reply_to_message_id'=>$message->message_id,
'reply_markup'=>json_encode([
'inline_keyboard'=>[
[['text'=>"✵⁞ المطور ⁞✵",'url'=>"t.me/$buy"]],
[['text'=>"✵⁞ تحديثات البوت ⁞✵",'url'=>"t.me/$namch"]],]])]);}}}}
$starttext = file_get_contents("data/start.txt");
if($text=="/start" and $starttext != null){
if($tc == "private"){
$st1 = file_get_contents("data/startlock.txt");
...
bot('sendphoto',['chat_id'=>$chat_id,'photo'=>"https://telegra.ph/file/14c7948ad180050fe16e4.jpg",
'caption'=>"$starttext",'parse_mode'=>'MarkDown','disable_web_page_preview'=>true,'reply_to_message_id'=>$message->message_id,
'reply_markup'=>json_encode([
'keyboard'=>[
[['text'=>"احاديث"]],
[['text'=>"دعاء"],['text'=>"رابط حذف"]],
[['text'=>"مبرمج السورس"],['text'=>"سورس"]],
[['text'=>"مقطع"],['text'=>"عبدالباسط"]],
[['text'=>"كت"],['text'=>"لو خيروك"]],
[['text'=>"تسبيح"],['text'=>"ذكر"]],
[['text'=>"سورة"],['text'=>"شعر"]],
[['text'=>"انمي"],['text'=>"استوري"]],
[['text'=>"بحث"],['text'=>"صور"]],
[['text'=>"فيلم"],['text'=>"ابراج"]],
[['text'=>"بنك"],['text'=>"قران"]],
[['text'=>"تلاوات"],['text'=>"كت بالصوت"]],
[['text'=>"مصحف"],['text'=>"أوقات الصلاة"]],
[['text'=>"افاتار شباب"],['text'=>"افاتار بنات"]],
],
'resize_keyboard'=>true
])]);}}}}
# --     Source ALMORTAGEL     --
...
file_put_contents("data/admin_user/$chat_id.txt",$str);
file_put_contents("data/admin_user/$chat_id/mange.txt",$str2);
$from_id_info = file_get_contents("data/ALMORTAGEL/$chat_id.txt");
$cas = file_get_contents("data/ALMORTAGEL/$chat_id/crlo.txt");
$cas1 = explode(" \n",$cas);
$str = str_replace($from_id,"",$from_id_info);
...
if($ti[1] && $rt){
if($status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) || in_array($from_id,$developer) || in_array($from_id,$eri) || in_array($from_id,$ALMORTAGEL) || in_array($from_id,$nazar)) {
bot('promoteChatMember',[
'chat_id'=>$chat_id,
'user_id'=>$re_id,
...
if ( $status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)) {
if ( $statusrt != 'creator' && !in_array($re_id,$Dev) && !in_array($re_id,$manger) && !in_array($re_id,$admin_user) && !in_array($re_id,$mmyaz) && !in_array($re_id,$developer) && !in_array($re_id,$ALMORTAGEL) && !in_array($re_id,$nazar) && !in_array($re_id,$eri)) {
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"⋄︙العضو - [$re_name](tg://user?id=$re_id)
⋄︙تم كـتـمه بـنـجاح",
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)) {
if ( $statusrt != 'creator' && $statusrt != 'administrator' && !in_array($re_id,$Dev) && !in_array($re_id,$manger) && !in_array($re_id,$admin_user) && !in_array($re_id,$mmyaz) && !in_array($re_id,$developer) && !in_array($re_id,$ALMORTAGEL) && !in_array($re_id,$nazar) && !in_array($re_id,$eri)) {
$add = $settings["information"]["added"];
if ($add == true){
bot('restrictChatMember',[
'user_id'=>$re_id,
...
,]);}
####
if($status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$nazar) or in_array($from_id,$eri)){
...
if($status != 'creator' or $status != 'administrator' and !in_array($from_id,$Dev) and !in_array($from_id,$developer) and !in_array($from_id,$ALMORTAGEL) and !in_array($from_id,$nazar) and !in_array($from_id,$eri)){
if($rt and $text == "ضع صوره" or $text == "ضع صورة" and $re_msd->photo){
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"*
...
bot('sendvoice',[
'chat_id'=>$chat_id,
'voice'=>"https://t.me/bsmaatt/" .rand(3,267),
  "caption"=>"✰ ⁞ تـم اختيـار اغنية لك",
'reply_to_message_id'=>$message_id,
'reply_markup'=>json_encode([
'inline_keyboard'=>[[['text'=>"- Source ALMORTAGEL",'url'=>"t.me/$namch"]],]])]);}}
#┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
$re = $update->message->reply_to_message;
...
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri) or in_array($from_id,$AUBEHAB) or in_array($from_id,$nazar)) {
...
• لااستطيع طردك لانك منشئ اساسي •
 ...
 if ($text =="اطردني" and in_array($from_id,$ALMORTAGEL)){
 bot("sendMessage",[
 "chat_id"=>$chat_id,
 "text"=>"
• لااستطيع طردك لانك منشئ •
...
if($text == "@all"){  
if ( $status == 'creator' or in_array($from_id,$Dev) or in_array($from_id,$manger) or in_array($from_id,$admin_user) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri) or in_array($from_id,$nazar)) {
...
⋄︙[تفاصيل الملف](https://t.me/$namch/2)
",'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
  'reply_to_message_id'=>$message_id,
'reply_markup'=>$inlinebutton,
 ]);
...
if($text == "تحميل $yotup"){
$keyboard = [];
$search = json_decode(file_get_contents("https://ggrff7hnn.ml/Yote/yotube_search_v2.1(ALMORTAGEL).php?search=".urlencode($yotup)),true);
for($b=1; $b <= 10; $b++){   
...
file_put_contents("ALMORTAGEL.ogg",$get);
bot('deleteMessage',[
'chat_id'=>$update->callback_query->message->chat->id,
'message_id'=>$update->callback_query->message->message_id,
]);
...
'chat_id'=>$chat_id2,
'message_id'=>$message_id2,
'voice'=>new CURLFile("ALMORTAGEL.ogg"),
'caption' =>"*⋄︙تم تنزيل الاغنية بشكل بصمة ☑️*",
'parse_mode'=>"MARKDOWN",
  'title'=>"$title",
     ]);
unlink("ALMORTAGEL.ogg");
}
############
...
if ($add == true) {
 ...
 'text'=>"⋄︙الملف ⋙ *Zhrafa*
⋄︙تم تفعيله في البوت بنجاح
⋄︙[تفاصيل الملف](https://t.me/$namch/3)
",'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
  'reply_to_message_id'=>$message_id,
'reply_markup'=>$inlinebutton,
 ]);
...
$k = file_get_contents("https://nsssar.tk/photo/API.php?almortagel=".$text);
bot('sendMessage',[
 'chat_id'=>$chat_id,
'text'=>"$k
 ـ────────────
...
⋄︙تم تفعيله في البوت بنجاح
⋄︙[تفاصيل الملف](https://t.me/$namch/4)
",'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
  'reply_to_message_id'=>$message_id,
'reply_markup'=>$inlinebutton,
 ]);
...
⋄︙تم تفعيله في البوت بنجاح
⋄︙[تفاصيل الملف](https://t.me/$namch/5)
",'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
  'reply_to_message_id'=>$message_id,
'reply_markup'=>$inlinebutton,
 ]);
...
⋄︙تم تفعيله في البوت بنجاح
⋄︙[تفاصيل الملف](https://t.me/$namch/7)
",'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
  'reply_to_message_id'=>$message_id,
'reply_markup'=>$inlinebutton,
 ]);
...
bot('sendMessage',[
'chat_id'=>$chat_id, 
'text'=>"⋄︙قائمة ملفات متجر سورس المرتجل
⋄︙الملفات المتوفره حاليا ↫ ⤈
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
⋄︙[ملف العاب اونلاين](https://t.me/$namch/3) ↫ ⤈
1~ : game ↬ ($gamejson)
⋄︙[ملف زخرفه](https://t.me/$namch/4) ↫ ⤈
2~ : Zhrafa ↬ ($Zhrafajson)
⋄︙[ملف تنبيه عند تغير الاسم](https://t.me/$namch/5) ↫ ⤈
3~ : ChangeName ↬ ($ChangeName)
⋄︙[ملف تنبيه عند تغير المعرف](https://t.me/$namch/6) ↫ ⤈
4~ : ChangeUser ↬ ($ChangeUser)
⋄︙[ملف تنبيه عند تغير الصورة](https://t.me/$namch/7) ↫ ⤈
...
file_put_contents("data/ALMORTAGEL.json",json_encode($coss));
}
}
...
[['text'=>'𝙱𝙾𝚃 𓆩ALMORTAGEL𓆪','url'=>'t.me/$userbot']],
]])
]); 
}
...
bot('sendphoto',[
'chat_id'=>$chat_id,
'photo'=>"https://t.me/Almortagel20_bot",
'caption' =>"┏•━•━•━ 『𝗦𝗢𝗨𝗥𝗖𝗘』 ━•━•━•┓
╏⍟ 𝚆𝙴𝙻𝙲𝙾𝙼𝙴 𝚃𝙾 
╏⍟ 𝙸𝙽𝙵𝙾 𝙳𝙴𝚅 𝗦𝗢𝗨𝗥𝗖𝗘  
╏⍟ $time
...
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[['text'=>"✵⁞ 𝙱𝙾𝚃 𝙳??𝚅 ⁞✵",'url'=>"t.me/Almortagel20_bot"]],
[['text'=>"✵⁞ 𝙳𝙴𝚅 ⁞✵",'url'=>"t.me/$buy"],['text'=>"✵⁞ 𝙲𝙷  ⁞✵",'url'=>"t.me/$chsource"]],
]])
]); 
}
#######
if( $text=="تحذير" && $rt){
if ( $status == 'creator' or $status == 'administrator' or in_array($from_id,$Dev) or in_array($from_id,$developer) or in_array($from_id,$ALMORTAGEL) or in_array($from_id,$eri) or in_array($from_id,$nazar)) {
if ($tc == 'group' | $tc == 'supergroup'){
if ( $statusrt != 'creator' && $statusrt != 'administrator' && !in_array($re_id,$Dev) && !in_array($re_id,$manger) && !in_array($re_id,$admin_user) && !in_array($re_id,$manger) && !in_array($re_id,$developer) && !in_array($re_id,$ALMORTAGEL) && !in_array($re_id,$eri) && !in_array($re_id,$nazar)) {
$add = $settings["information"]["added"];
if ($add == true) {
...
$fr = file_get_contents("https://abu-ehab-vip.ml/Abuehab/api(ALMORTAGEL)translate.php?Language=ar&TO=fa&text=".$s);
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"$fr",
'reply_to_message_id'=>$message->message_id,
...
$enar = file_get_contents("https://abu-ehab-vip.ml/Abuehab/api(ALMORTAGEL)translate.php?Language=en&TO=ar&text=".$H);
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"$enar",
'reply_to_message_id'=>$message->message_id,
...
$arfr = file_get_contents("https://abu-ehab-vip.ml/Abuehab/api(ALMORTAGEL)translate.php?Language=fa&TO=ar&text=".$Q);
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"$arfr",
'reply_to_message_id'=>$message->message_id,
...
$aren = file_get_contents("https://abu-ehab-vip.ml/Abuehab/api(ALMORTAGEL)translate.php?Language=ar&TO=en&text=".$W);
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"$aren",
'reply_to_message_id'=>$message->message_id,
...
$artr = file_get_contents("https://abu-ehab-vip.ml/Abuehab/api(ALMORTAGEL)translate.php?Language=ar&TO=tr&text=".$E);
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"$artr",
'reply_to_message_id'=>$message->message_id,
...
$trar = file_get_contents("https://abu-ehab-vip.ml/Abuehab/api(ALMORTAGEL)translate.php?Language=ar&TO=tr&text=".$R);
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"$trar",
'reply_to_message_id'=>$message->message_id,
...
$entr = file_get_contents("https://abu-ehab-vip.ml/Abuehab/api(ALMORTAGEL)translate.php?Language=en&TO=tr&text=".$Y);
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"$entr",
'reply_to_message_id'=>$message->message_id,
...
$tren = file_get_contents("https://abu-ehab-vip.ml/Abuehab/api(ALMORTAGEL)translate.php?Language=tr&TO=en&text=".$T);
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"$tren",
'reply_to_message_id'=>$message->message_id,
...
file_put_contents("data/ALMORTAGEL.ogg",file_get_contents($file));
bot('sendvoice',[
'chat_id'=>$chat_id,
'voice'=>new CURLFile("data/ALMORTAGEL.ogg"),
'caption'=>"⋄︙تم تحويل الفيديو ← بصمة",
]);
unlink("data/ALMORTAGEL.ogg");
}
...
file_put_contents("data/ALMORTAGEL.mp3",file_get_contents($file));
bot('sendaudio',[
'chat_id'=>$chat_id,
'audio'=>new CURLFile("data/ALMORTAGEL.mp3"),
'caption'=>"⋄︙تم تحويل البصمة ← اغنية",
]);
unlink("data/ALMORTAGEL.mp3");
}
...
file_put_contents("data/ALMORTAGEL.png",file_get_contents($file));
bot('sendsticker',[
'chat_id'=>$chat_id,
'sticker'=>new CURLFile("data/ALMORTAGEL.png"),
]);
unlink("data/ALMORTAGEL.png");
}
...
file_put_contents("data/ALMORTAGEL.jpg",file_get_contents($file));
bot('sendphoto',[
'chat_id'=>$chat_id,
'photo'=>new CURLFile("data/ALMORTAGEL.jpg"),
'caption'=>"⋄︙تم تحويل الملصق ← صوره",
]);
unlink("data/ALMORTAGEL.jpg");
}
#######
...
$pirvate = explode("\n",file_get_contents("Fri3nd_s/pirvate.txt"));
$forward = $update->message->forward_from;
$mabALMORTAGEL = count($pirvate)-1;
$groALMORTAGEL = count($groups)-1;
...
⋄︙يوزره ← @$casss
--------------------------------
⋄︙الاعضاء ← $mabALMORTAGEL
⋄︙الكروبات ← $groALMORTAGEL
⋄︙قام بدخول بوتك الان
⋄︙الوقت ← $times
"]);} 
...
⋄︙تغير الايدي ↫ تغ
ꔹ┉ ┉ ┉ ┉ ┉ ┉ ┉ꔹ
[Source Almortagel](https://t.me/$namch)",
'parse_mode'=>"markdown",'reply_to_message_id'=>$message_id,
 ]);
 } 
 #---------------#
...
#-----------------------#
$bot_id = "5688348631";
$ALMORTAGEL = file_get_contents("data/ALMORTAGEL.json");
$ALMORTAGELs = file_get_contents("data/ALMORTAGELs.json");
if($text=="ضع ترحيب عام"){
file_put_contents("data/ALMORTAGELs.json","$from_id");
if( in_array($from_id,$Dev)){
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
if($text and $ALMORTAGEL =="ALMORTAGEL" and $ALMORTAGELs == $from_id){
if( in_array($from_id,$Dev)){
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
...
$keyboard = [];
$search = json_decode(file_get_contents("https://ggrff7hnn.ml/vip/vip2/yotube_search_v2.1(ALMORTAGEL).php?search=".urlencode($tahmel)),true);
for($b=1; $b <= 10; $b++){   
$keyboard['inline_keyboard'][] = [['text'=>$search['results'][$b]['title'], 'callback_data'=>"jaemax##".$search['results'][$b]['url']]];
$reply_markup=json_encode($keyboard);
}
...
//start
if($text == "/bahs" or $text == "عودة" or $text == "اوامر"){
  $json ["$id"]["save"] = "start";
...
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
[['text' => "✧ - قناة عالم البرمجه ♥", 'url' =>"https://t.me/botatiiii"]],
]])]);}
...
bot("sendvoice",[
"chat_id"=>$chat_id,
"voice"=>"https://t.me/EIEI06/" . rand(24,618),
"caption"=>"✰ ⁞ تـم اختيـار تلاوات لـك", 
'reply_to_message_id'=>$message->message_id,
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
...
"caption"=>"[✧ - نسألكم الدعاء 💙](https://t.me/$namch)", 
'reply_to_message_id'=>$message->message_id,
'parse_mode'=>"MarkDown",
'reply_markup'=>json_encode([
'inline_keyboard'=>[
...
if($text == "انمي" or $text == "انميي"){
bot("sendphoto",[
"chat_id"=>$chat_id,
"photo"=>"https://t.me/LoreBots7/" . rand(1,153),
"caption"=>"✰ ⁞ تـم اختيـار انمي لـك", 
'reply_to_message_id'=>$message->message_id,
'parse_mode'=>"MarkDown",
'reply_markup'=>json_encode([
'inline_keyboard'=>[
[[ 'text' => 'انمي اخر',callback_data => 'appss' ]],
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
[['text' => "✧ - قناة عالم البرمجه ♥", 'url' =>"https://t.me/botatiiii"]],
]])]);}
if($date == "appss"){
...
if($text == "استوري" or $text == "استوريي"){
  bot("sendvideo",[
  "chat_id"=>$chat_id,
  "video"=>"https://t.me/yoipopl/" . rand(1,147),
  "caption"=>"✰ ⁞ تـم اختيـار استوري لك",
  'reply_to_message_id'=>$message->message_id,
  'reply_markup'=>json_encode([
      'inline_keyboard'=>[
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
...
if($text == "افاتار بنات"){
  bot("sendphoto",[
  "chat_id"=>$chat_id,
  "photo"=>"https://t.me/vvyuol/" . rand(2,215),
  "caption"=>"✰ ⁞ تـم اختيـار افاتار بنات لك",
  'reply_to_message_id'=>$message->message_id,
  'reply_markup'=>json_encode([
      'inline_keyboard'=>[
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
...
if($text == "افاتار شباب"){
  bot("sendphoto",[
  "chat_id"=>$chat_id,
  "photo"=>"https://t.me/vgbmm/" . rand(2,176),
  "caption"=>"✰ ⁞ تـم اختيـار افاتار شباب لك",
  'reply_to_message_id'=>$message->message_id,
  'reply_markup'=>json_encode([
      'inline_keyboard'=>[
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
...
if($text == "فيلم" or $text == "فيلمك" or $text == "م"){
  bot("sendvideo",[
  "chat_id"=>$chat_id,
  "video"=>"https://t.me/gyigkk/" . rand(1,34),
  "caption"=>"✰ ⁞ تـم اختيـار فيلم لـك",
  'reply_to_message_id'=>$message->message_id,
  'reply_markup'=>json_encode([
      'inline_keyboard'=>[
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
...
'inline_keyboard' => [
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],
[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
[['text' => "✧ - القرآن الكريم ♥", 'url' =>"https://t.me/$namch"]],
]])
]);
}
if($text < 10){
...
if($text == "كت" or $text == "كت تويت" or $text == "تويت"){
$x3raqe = file_get_contents("https://hmss.store/api/kt.php");
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"*$x3raqe*",
'parse_mode'=>"Markdown",
...
$message = $update->message;
$ser = $message->text;
$mlg = $update->callback_query->message->message_id;
$files = $message->chat->id;
$slo = $update->callback_query->data;
$msg = $update->callback_query->message->chat->id;
$source = file_get_contents("http://lmasat.tk/af/As.php");
//code//
if($ser == "التسبيح" or $ser == "تسبيح" or $ser == "تس"){
bot('sendmessage', [
'chat_id' => $files,
'text' => "$source",
'reply_to_message_id'=>$message->message_id,
'reply_markup'=>json_encode([
'inline_keyboard'=>[
[[ 'text' => 'تسبيح اخر🤍',callback_data => 'apps' ]],
     ]])
   ]);
}
//تكرار التسبيح//
...
if($text == "كت بالصوت" or $text == "كت صوت" or $text == "كتص"){
bot("sendvoice",[
"chat_id"=>$chat_id,
"voice"=>"https://t.me/aslmiattyyu/" . rand(146,194),
"caption"=>"✰ ⁞ تـم اختيـار هذا الكت الصوتي لك", 
'reply_to_message_id'=>$message->message_id,
'parse_mode'=>"Markdown",
'reply_markup'=>json_encode([
'inline_keyboard'=>[
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
[['text' => "✧ - قناة عالم البرمجه ♥", 'url' =>"https://t.me/botatiiii"]],]])]);}
if($text == "خيروك" or $text == "لو خيروك" or $text == "خير"){
$xAlmortagel = file_get_contents("https://lmasat.tk/af/khrok.php");
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"*$xAlmortagel*",
'parse_mode'=>"Markdown",
...
if($text == "خطب" or $text == "الخطب" or $text == "خطبة"){
$sAlmortagel = file_get_contents("https://nsssar.tk/nizk/kutab.php");
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"*$sAlmortagel*",
'parse_mode'=>"Markdown",
...
if($text == "شعر" or $text == "اشعار"){
bot("sendvoice",[
"chat_id"=>$chat_id,
"voice"=>"https://t.me/L1BBBL/" . rand(7,622),
"caption"=>"✰ ⁞ تـم اختيـار هذا الشعر لك", 
'reply_to_message_id'=>$message->message_id,
'parse_mode'=>"MarkDown",
'reply_markup'=>json_encode([
'inline_keyboard'=>[
...
$mshark = json_decode(file_get_contents("mshark.json"), true);
$name = $message->from->first_name;
$update = json_decode(file_get_contents("php://input"));
$message = $update->message;
$txt = $message->caption;
...
if($text == "البنك" or $text == "بنك") {
	bot('sendMessage',[
   'chat_id'=>$chat_id,
   'text'=>"
✜ اوامر البنك سورس المرتجل
...
$id_acc = substr(str_shuffle('12345689807891234568980789'),1,15);
 if($text == "بنك المرتجل" or $text == "بنك الحظ" or $text == "بنك مصر" ){
...
'reply_to_message_id'=>$message_id,
   ...
   'chat_id'=>$chat_id,
   'text'=>"
⇜ الحد الادنى المسموح هو 200 جنيه 
", 
'parse_mode'=>"markdown",
'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message_id,
...
المبلغ: `". $binks["3ddT7"][$from_id][$chat_id]. "` جنيه  🎉
", 
'parse_mode'=>"markdown",
'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message_id,
...
if($text == "فلوسي") {
   ...
   'chat_id'=>$chat_id,
   'text'=>"
⇜ فلوسك `".$binks["acount_flos"][$from_id]. "` جنيه  🥇
", 
'parse_mode'=>"markdown",
'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message_id,
...
⇜  كفو فزت بالمضاربه!
⇜ نسبة الفوز ↢ $rnd
⇜ مبلغ الفوز ↢ ( $mthrb1  جنيه ) 
⇜ فلوسك صارت ↢ ( $aft_coin جنيه ) 
", 
'parse_mode'=>"markdown",
'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message_id,
...
⇜ نسبة الخسارة ↢ $rnd
⇜ مبلغ الخسارة ↢ ( $mthrb1 جنيه ) 
⇜ فلوسك صارت ↢ ( $coins جنيه ) 
", 
'parse_mode'=>"markdown",
'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message_id,
...
[✧ - نسألكم الدعاء 🤍](https://t.me/$namch)
",
        'parse_mode'=>"MarkDown",
        'reply_markup'=> json_encode([
    'inline_keyboard'=>[
        [['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],
        [['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
        [['text'=>"✧ - اوقات الصلاة ⏱️",'web_app'=>['url'=>"https://dev-$namch.pantheonsite.io/time"]]]
    ]
    ])
    ]);
}
...
bot('sendmessage',[
'chat_id'=>$chat_id,
"text" =>"*❍ 𝙷𝙸, 𝙸 𝙰𝙼 𝙰 𝚂𝙴𝙼𝚂𝙼𝙴 𝚁𝙴𝙿𝙻𝚈 𝙱𝙾𝚃 𝙳𝙴𝚅𝙴𝙻𝙾𝙿𝙴𝙳 𝚆𝙸𝚃𝙷 𝙰𝚁𝚃𝙸𝙵𝙸𝙲𝙸𝙰𝙻 𝙸𝙽𝚃𝙴𝙻𝙻𝙸𝙶𝙴𝙽𝙲𝙴 .*",
'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message_id, 
]);
}
...
if($text != "سمسمي"){
$api = json_decode(file_get_contents("https://ava-tar.online/bot/test?jack=".$text),true);
$api_text =$api["origin_sentence"];
bot('sendMessage',[ 
'chat_id'=>$chat_id, 
'text'=>"*$api_text*", 
'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
...
bot('sendvideo',[
'chat_id'=>$chat_id,
'video'=>"https://telegra.ph/file/66253b50dc3663176317d.mp4",
'caption'=>"♡ ⁞ اهلا بكم احبتي🌜
...
"reply_markup"=>json_encode([ 
"inline_keyboard"=>[
[['text'=>'♡ ⁞ المبرمج','url'=>'t.me/Almortagel_12']],
[['text'=>'♡ ⁞ قناة البوت','url'=>'t.me/$namch']],
[['text'=>"♡ ⁞ شارك البوت",'switch_inline_query'=>"♡ ⁞ بوت المرتجل لحمايه المجموعات شارك البوت"]],
]]) 
]); 
}
$tilitet="♽ قم بمشاركة البوت ♽";
$index="<h2>@Almortagel_12</link>";
$name = $message->from->first_name;
...
'parse_mode'=>"html",
'disable_web_page_preview'=>true,
'reply_to_message_id'=>$message->message_id,'message_text'=>"✧ - مرحبا بك في بوت المرتجل لحمايه المجموعات 
\nوضيفتي حماية المجموعات من السبام والتفليش ..."],
'reply_markup'=>['inline_keyboard'=>[
[['text'=>"♽ لتصفح البوت اضغط هنا ♽ ",'url'=>"https://telegram.me/$userbot?start"]],[['text'=>"$tilitet ",'switch_inline_query'=>"$from_id"]],
...
if($text == "قنوات المرتجل"){
bot('sendMessage',[ 
'chat_id'=>$chat_id, 
'text'=>"*👮 - مرحبا *[$name](tg://user?id=$id)*

- اليك قائمة قنوات المرتجل♡
...
"inline_keyboard"=>[
[['text' => "✧ - المطور 🌐", 'url' =>"https://t.me/Almortagel_12"]],[['text' => "✧ - قناة مطور البوت", 'url' =>"https://t.me/$namch"]],
[['text' => "✧ - قناة تطوير برمجية", 'url' =>"https://t.me/gobzela"]],
[['text' => "✧ - قناة اذاعه القران من القاهرة ♥", 'url' =>"https://t.me/AljanahYaAllahh"]],
]])]);}
$update = json_decode(file_get_contents('php://input'));
$message = $update->message;
$from_id = $message->from->id;
...
$useradmin = bot("getChat?chat_id=$admin")->result->username; 
...
if($text == "قنواتي"){
if($update->channel_post){
$mad = bot("getChat", ["chat_id" => $chat_id])->result->title;
$x = $mad;
}else{
$x = "*[$name](tg://user?id=$from_id)*";
}
bot('sendMessage',[ 
'chat_id'=>$chat_id, 
'text'=>"*👮 - مرحبا *[$name]*
...
'reply_markup'=>json_encode([
'inline_keyboard'=>[
[['text'=>'- الاوامر .' ,callback_data=>"cmd"],['text'=>'- حول .' ,callback_data=>"infioe"]],
[['text'=>"- قناة السورس .",'url'=>"t.me/$namch"],['text'=>"- المطور .",'url'=>"t.me/Almortagel_12"]],
[['text'=>"- اضف البوت لقناتك .",'url'=>"https://t.me/$username?startchannel"]],
]])
]);
}
if($data == "back" ){
...
"reply_markup"=>json_encode([
"inline_keyboard"=>[
[['text'=>'- الاوامر .' ,callback_data=>"cmmd"],['text'=>'- حول .' ,callback_data=>"infioe"]],
[['text'=>"- قناة السورس .",'url'=>"t.me/$namch"],['text'=>"- المطور .",'url'=>"t.me/Almortagel_12"]],
[['text'=>"- اضف البوت لقناتك .",'url'=>"https://t.me/$username?startchannel"]],
]])]);}
if($data == "infioe" ){
...
if( $text =="فتح سمسمي" or $text == "تفعيل سمسمي" and $from_id == $admin){
	bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"*⌯⁞  تم تفعيل وضع سمسمي بنجاح*",'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
  'reply_to_message_id'=>$message_id,
 ]);
file_put_contents("smsmi $chat_id.txt","ys");
}
if ( $text =="تعطيل سمسمي" or $text == "قفل سمسمي" and $from_id == $admin){
	bot('sendmessage',[
	'chat_id'=>$chat_id,
	'text'=>"*⌯⁞  تم تعطيل وضع سمسمي بنجاح*",'parse_mode'=>"markdown",'disable_web_page_preview'=>true,
  'reply_to_message_id'=>$message_id,
'reply_markup'=>$inlinebutton,
 ]);
unlink("smsmi $chat_id.txt");
}
if($text != "سمسم" && $smsmi == "ys"){
$amrrdamr = file_get_contents("https://z63k.tk/API/smim.php?RD=".urlencode($text), true);
bot('SendMessage',[
'chat_id'=>$chat_id,
'text'=>"$amrrdamr",
'reply_to_message_id'=>$message->message_id,
...
$message_id = $update->inline_query->inline_message_id; 
...
if( $text_inline && $status == "left") { 
bot('answerInlineQuery',[ 
'inline_query_id'=>$update->inline_query->id,
'results' => json_encode([['type'=>'article', 
'description'=>"", 
'id'=>base64_encode(rand(5,555)), 
'title'=>" طـريقه استخدام البـوت 🔥",
'thumb_url'=>'t.me/$a',
'input_message_content'=>['parse_mode'=>'HTML',
'message_text'=>"معرف البوت ثم الرساله ثم معرف الشخص"], 
]])]);return false;}
...
mkdir("code");
file_put_contents("code/$rand.txt","$txt");
$userh=str_replace(' ',"", $us);
$userh =strtolower($userh);
$mb_str=mb_strlen($text,"utf-8");#str#$mb_str=mb_strlen($text_inline,"utf-8");
if(!preg_match("/(@all)/",$nn) ){
bot('answerInlineQuery',[ 
'inline_query_id'=>$update->inline_query->id,
'results'=>json_encode([[ 
'type'=>'article',
'id'=>base64_encode(rand(5,555)), 
'title'=>" ⚘ ⁞ هذه الهمسه السريه لك عزيزي { $nn } انته وحدك تسطيع رؤيتها ♥", 
'description'=>"⚘ ⁞ هـو فقـط مـن يستطـيع رويـتها 🌚",'thumb_url'=>'t.me/$a','input_message_content'=>['parse_mode'=>'HTML',
'message_text'=>"⚘ ⁞ همسه سرية الئ { $nn } 🧚🏻‍♀️"],
'reply_markup'=>['inline_keyboard'=>[ 
[['text'=>'⚘ ⁞ اضهار الهمسه 🪅','callback_data'=>$rand.'to'.$userh.'to'.$from_id]],]]]])]);
}else{
bot('answerInlineQuery',[ 
'inline_query_id'=>$update->inline_query->id,
'results'=>json_encode([['type'=>'article',
'id'=>base64_encode(rand(5,555)), 
'title'=>"⚘ ⁞ همسه عامه للكل 💚", 
'description'=>"⚘ ⁞ الجميع يستطيع فتحها 🍒",'thumb_url'=>'t.me/$a','input_message_content'=>['parse_mode'=>'HTML',
'message_text'=>"⚘ ⁞ هذه الهمسه للجميع 🤍"], 
'reply_markup'=>['inline_keyboard'=>[ 
[['text'=>'فتح الهمسه 🍓','callback_data'=>$rand.'to'.$userh.'to'.$from_id]],]]]])]);}
}else{
$rand=rand(111111,999999);
mkdir("code");
...
$res[$i] = ['type'=>'article',
'id'=>base64_encode(rand(5,555)), 
'title'=>"⚘ ⁞ هذه الهمسه السريه لك عزيزي { $mem } انته وحدك تسطيع رؤيتها 💛", 
'description'=>"⚘ ⁞ هـو فقـط مـن يستطـيع رويـتها 👀",'thumb_url'=>'t.me/$a','input_message_content'=>['parse_mode'=>'HTML',
'message_text'=>"⚘ ⁞ همسة سرية الى { $mem } 🌸"], 
'reply_markup'=>['inline_keyboard'=>[ 
[['text'=>'⚘ ⁞ اضهار الهمسه 🪅','callback_data'=>$rand.'to'.$mem.'to'.$from_id]],]]];
}else{
$res[$i] = ['type'=>'article',
'id'=>base64_encode(rand(5,555)), 
'title'=>"⚘ ⁞ هذه الهمسه للجميع 🕊",
'description'=>"⚘ ⁞ هذه الهمسه عامه الجميع يستطيع رؤيتها 🌼",
'thumb_url'=>'t.me/$a',
'input_message_content'=>['parse_mode'=>'HTML',
'message_text'=>"⚘ ⁞ هذه الهمسه للجميع 🕊"], 
'reply_markup'=>['inline_keyboard'=>[ 
[['text'=>'فتح الهمسه 🌺','callback_data'=>$rand.'to'.$mem.'to'.$from_id]],]]];}
...
$txt=file_get_contents("code/$rand.txt");
if(in_array($user,$e) or in_array($from_id,$e) or $from_id==$custom or $from_id== $admin2 or in_array("all",$e)){
...
'show_alert'=>true]); 
 ...
 bot('answerCallbackQuery',[ 
'callback_query_id'=>$update->callback_query->id, 
'text'=>"⚘ ⁞ عذرا هذه الهمسه طويله سيتم ارسالها لك في الخاص ☘️", 
'show_alert'=>true]);
...
$gameJson = json_decode(file_get_contents("gameJson.json"),true);
$link = "https://t.me/LANBOT1/".rand(2,224);

 if($text == "صور" ){
  bot('sendphoto', [
'chat_id'=>$chat_id,
'photo'=>$link,
'caption'=>"شتشوف بالصوره ؟",
'parse_mode'=>"markdown",
]);
$gameJson["photo"][$chat_id]  = html_entity_decode(get_meta_tags($link)['twitter:description']);
$gameJson["timeph"][$chat_id]  = $message->date;
$gameJson= json_encode($gameJson,32|128|265);
file_put_contents("gameJson.json",$gameJson);
}


  if($gameJson["photo"][$chat_id]){
  $text = "$text\n";
$afterTimr = time()-$gameJson["timeph"][$chat_id];
  if($text == $gameJson["photo"][$chat_id]) {
...
*
༄
صح عليك -* [$name](tg://user?id=$from_id)
*⏰الوقت: $afterTimr ثانيه
༄*
...
$gameJson["timeph"][$chat_id]  = null;
$gameJson["photo"][$chat_id] = null;
$gameJson= json_encode($gameJson,32|128|265);
file_put_contents("gameJson.json",$gameJson);
}else{
 bot('sendMessage', [
...
if($text and $kindi =="nam" ){
file_put_contents("kindi.txt","");
$k = file_get_contents("https://nsssar.tk/photo/API.php?text=".$text);
bot('sendmessage',[
 'chat_id'=>$chat_id,
'text'=>"$k ـ────────────
⌯ تمت الزخرفة بنجاح لـ ⋙ $text ؛
...
,]);}
$ali = rand(13,3431);
$msg = "https://t.me/TheIslamicProphet/$ali";
if($text == "حديث" or $text == "احاديث نبويه" or $text == "احاديث"){
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>html_entity_decode(get_meta_tags($msg)['twitter:description']),
'reply_to_message_id'=>$message_id,
]);
}
...
$url = "https://telegra.ph/file/1bde733d99eb3ef0d507f.mp4";
if(preg_match('/^(تخ)(.*)/',$text)){ 
$text = str_replace('تخ','',$text);
bot("sendvideo",[ 
"chat_id"=>$chat_id, 
"video"=>$url,
"caption"=>"
قتل ↫ [$for](tg://user?id=$from_id)
الضحيه دا 😢 ↫[$text](tg://user?id=".$ALI->from->id.")
...
"reply_markup"=>json_encode([ 
"inline_keyboard"=>[  
[['text'=>'المقتول 🔪','url'=>"tg://user?id=".$ALI->from->id]],  
]]) 
]); 
}
$for = $message->from->first_name;
$from_id = $message->from->id;
$Almortagel = $message->reply_to_message;
$url = "https://telegra.ph/file/53eae283620f18b6fae2a.mp4";
if(preg_match('/^(تف)(.*)/',$text)){ 
$text = str_replace('تف','',$text);
bot("sendvideo",[ 
"chat_id"=>$chat_id, 
"video"=>$url,
"caption"=>"
تف ↫ [$for](tg://user?id=$from_id)
الضحيه دا 😢 ↫[$text](tg://user?id=".$ALI->from->id.")
اععع اي القرف دا
",
'parse_mode'=>'markdown',
...
"reply_markup"=>json_encode([ 
"inline_keyboard"=>[  
[['text'=>'المجني عليه 😢','url'=>"tg://user?id=".$Almortagel->from->id]],  
]]) 
]); 
}
if($text=="/start" &&  $tc == "private" or $text=="كيب" &&  $tc == "private"){
bot('sendmessage',[
'chat_id'=>$chat_id,
'text'=>"
",'parse_mode'=>"MarkDown",
